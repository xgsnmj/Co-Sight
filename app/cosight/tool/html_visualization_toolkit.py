...
def configure_matplotlib_fonts():
    ...
    if not os.path.exists(font_path):
        try:
            ...
            logger.error(f"下载字体时出错: {str(e)}", exc_info=True)
            # 使用系统默认字体作为后备
            font_path = None
    ...
    for ext in ['*.txt', '*.md', '*.json', '*.csv']:
        ...
        for file_path in file_paths:
            try:
                ...
                logger.error(f"读取文件 {file_path} 时出错: {str(e)}", exc_info=True)
...
"""
    ...
    if response:
        try:
            ...
            logger.error(f"解析大纲JSON时出错: {str(e)}", exc_info=True)
...
def convert_markdown_to_html(text):
    ...
    try:
        ...
        logger.error(f"转换Markdown到HTML时出错: {str(e)}", exc_info=True)
        # 如果转换失败，返回原始文本，但将基本的Markdown标题和格式转换为HTML
        try:
            # 转换标题
            text = re.sub(r'^(#{1,6})\s+(.+?)$', lambda m: f'<h{len(m.group(1))}>{m.group(2)}</h{len(m.group(1))}>',
...
"""
            ...
            with progress_lock:
                logger.info(
                    f"处理子章节 [{section_idx + 1}.{subsection_idx + 1}]: {subsection['title']} ({processed_count + 1}/{total_subsections})")
            ...
            with progress_lock:
                processed_count += 1
                logger.info(
                    f"完成子章节 [{section_idx + 1}.{subsection_idx + 1}]: {subsection['title']} ({processed_count}/{total_subsections})")

            return result
        except Exception as e:
            logger.error(f"处理子章节 {subsection['title']} 时出错: {str(e)}", exc_info=True)
            return {
                'title': subsection['title'],
...
"""
        ...
        try:
            ...
            if visualization_info['suitable_for_visualization']:
                ...
                visualization_info['data_dimensions'] = [
                    visualization_info.get('variables', {}).get('x_axis', '类别' if is_chinese else 'Category')]
                ...
                if 'title' not in visualization_info:
                    visualization_info['title'] = visualization_info.get('theme',
                                                                         '数据可视化' if is_chinese else 'Data Visualization')
...
def generate_sample_data(visualization_info):
    ...
    try:
        ...
        logger.error(f'rasie error: {str(e)}', exc_info=True)
        return None
            ...
            if sample_data.shape[1] >= 2 and sample_data.shape[0] >= 3:
                ...
                fig = go.Figure(data=[go.Sankey(
                    ...
                    link=dict(
                        ...
                        color=[
                            f'rgba({int(custom_palette[0][1:3], 16)},{int(custom_palette[0][3:5], 16)},{int(custom_palette[0][5:7], 16)},0.3)'
                            for _ in links]
                    )
                )])
...
"""
        ...
        try:
            ...
            logger.error(f"解析指标JSON时出错: {str(e)}", exc_info=True)
            return []
        except Exception as e:
            logger.error(f"处理指标时出错: {str(e)}", exc_info=True)
            return []
    except Exception as e:
        logger.error(f"提取关键指标时出错: {str(e)}", exc_info=True)
        return []
...
def save_html_report(html_content, report_name="report"):
    ...
    try:
        ...
        logger.error(f"保存HTML报告时出错: {str(e)}", exc_info=True)
        return None
        ...
        if include_charts:
            ...
            def process_visualization(i, section, j, subsection):
                ...
                try:
                    ...
                    if 'all' in chart_types:
                        chart_match = True
                    elif chart_type in ['折线图', '线图', 'line chart', 'line graph',
                                        'line plot'] and 'line' in chart_types:
                        chart_match = True
                    elif chart_type in ['柱状图', '条形图', 'bar chart', 'bar graph',
                                        'histogram'] and 'bar' in chart_types:
                        chart_match = True
                    elif chart_type in ['饼图', '圆饼图', 'pie chart'] and 'pie' in chart_types:
                        chart_match = True
                    elif chart_type in ['散点图', 'scatter plot', 'scatter graph'] and 'scatter' in chart_types:
                        chart_match = True
                    elif chart_type in ['雷达图', 'radar chart', 'radar plot',
                                        'spider chart'] and 'radar' in chart_types:
                        chart_match = True
                    elif chart_type in ['热力图', 'heatmap', 'heat map'] and 'heatmap' in chart_types:
                        chart_match = True
                    elif chart_type in ['气泡图', 'bubble chart', 'bubble plot'] and 'bubble' in chart_types:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        logger.error(f"获取可视化结果时出错: {str(e)}", exc_info=True)
...
def main(params=None):
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            logger.error(f"报告生成失败: {result['message']}")

        return result
    except Exception as e:
        logger.error(f"执行HTML报告生成工具时出错: {str(e)}", exc_info=True)
        return {
            "status": "error",
